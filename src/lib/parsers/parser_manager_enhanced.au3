...
Func ParserManager_DetectLogFormat($sFilePath)
    ...
    ; Prüfe auf unvollständige JSON-Einträge mit dem kombinierten Pattern
...
Func ParserManager_TestIncompleteJson($sFilePath)
    ...
    ; Suche direkt nach unvollständigen JSON-Strings mit dem Pattern
    Local $aIncompleteMatches = StringRegExp($sContent, '\{[^\}]*?$', 3)
