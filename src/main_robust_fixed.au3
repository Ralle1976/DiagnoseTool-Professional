    ; Globale Datenbank-Variable setzen, wenn noch nicht vorhanden
    $g_sCurrentDB = ""#RequireAdmin
...
#include "lib/sql_editor_utils.au3"  ; SQL-Editor Hilfsfunktionen mit Autovervollständigung
...
Func _CreateMainGUI()
    ...
    $g_hGUI = GUICreate("Diagnose Tool (Robuste Logdatei-Unterstützung)", 1200, 700)
    ...
    ; Wichtig: Erst müssen die Button-IDs global für den persistenten Editor verfügbar gemacht werden
...
Func WM_NOTIFY($hWnd, $iMsg, $wParam, $lParam)
    ...
    If $g_bSQLEditorMode And $g_hAutoCompleteList <> 0 And $hWndFrom = $g_hAutoCompleteList Then
        Switch $iCode
    ...
    While 1
        ...
        Switch $iMsg
            ...
            Case $idFileLogOpen, $idBtnLogOpen
                Local $sFile = FileOpenDialog("Logdatei öffnen", $g_sLastDir, "Logdateien (*.log;*.txt)|Alle Dateien (*.*)", $FD_FILEMUSTEXIST)
                If Not @error Then
                    $g_sLastDir = StringLeft($sFile, StringInStr($sFile, "\", 0, -1))
