...
; Log-Viewer-Funktionen werden direkt von den benötigten Modulen geladen
...
; ===============================================================
; Fehlende Funktionen für main.au3
; ===============================================================

; Öffnet das Entpack-Verzeichnis im Explorer
Func _OpenExtractDirectory()
    _LogInfo("Öffne Entpack-Verzeichnis")

    ; Globale Variable prüfen für das aktuelle Entpack-Verzeichnis
    Local $sExtractDir = ""

    ; Versuche verschiedene globale Variablen zu finden
    If IsDeclared("g_sCurrentExtractPath") Then
        $sExtractDir = Eval("g_sCurrentExtractPath")
    ElseIf IsDeclared("g_sExtractPath") Then
        $sExtractDir = Eval("g_sExtractPath")
    ElseIf IsDeclared("g_sLastExtractPath") Then
        $sExtractDir = Eval("g_sLastExtractPath")
    EndIf

    ; Falls keine globale Variable, Standard-Entpack-Pfad verwenden
    If $sExtractDir = "" Then
        $sExtractDir = @TempDir & "\DiagnoseTool_Extract"
    EndIf

    ; Prüfen ob Verzeichnis existiert
    If Not FileExists($sExtractDir) Then
        _LogWarning("Entpack-Verzeichnis existiert nicht: " & $sExtractDir)

        ; Alternatives Verzeichnis suchen
        Local $sAlternativePath = @ScriptDir & "\extracted"
        If FileExists($sAlternativePath) Then
            $sExtractDir = $sAlternativePath
            _LogInfo("Verwende alternatives Entpack-Verzeichnis: " & $sExtractDir)
        Else
            MsgBox(48, "Hinweis", "Es wurde noch kein ZIP-Archiv extrahiert oder das Entpack-Verzeichnis ist nicht mehr verfügbar." & @CRLF & @CRLF & _
                   "Gesuchtes Verzeichnis: " & $sExtractDir)
            Return False
        EndIf
    EndIf

    ; Verzeichnis im Explorer öffnen
    _LogInfo("Öffne Verzeichnis im Explorer: " & $sExtractDir)
    ShellExecute("explorer.exe", $sExtractDir)

    ; Statusmeldung
    GUICtrlSetData($g_idStatus, "Entpack-Verzeichnis geöffnet: " & StringRegExpReplace($sExtractDir, "^.*\\", ""))
