...
Func _ForceSetSQLText($sText)
    ...
    ; 1. Methode: Direktes Löschen des Inhalts und dann Setzen des neuen Textes
    _GUICtrlRichEdit_SetText($g_hSQLRichEdit, "")
    Sleep(10)
    _GUICtrlRichEdit_SetText($g_hSQLRichEdit, $sText)
    Sleep(10)
    
    ; 2. Methode: Text mit ControlSetText setzen
    ControlSetText($g_hGUI, "", $g_hSQLRichEdit, $sText)
    Sleep(10)
    
    ; 3. Methode: Text mit WM_SETTEXT-Nachricht setzen
    DllCall("user32.dll", "lresult", "SendMessageW", "hwnd", $g_hSQLRichEdit, "uint", $WM_SETTEXT, "wparam", 0, "wstr", $sText)
    Sleep(10)
    
    ; 4. Methode: Text mit Tastaturemulation setzen (STRG+A, DEL, Paste)
...
Func _TogglePersistentSQLEditor($bActivate)
    ...
    If $bActivate Then
        ...
        ; RichEdit anzeigen - zusätzliche Maßnahmen zur Sicherstellung
        If IsHWnd($g_hSQLRichEdit) Then
            _WinAPI_ShowWindow($g_hSQLRichEdit, @SW_SHOW)
            
            ; Wichtig: Erst richtige Größe setzen, dann aktualisieren
            ControlMove($g_hGUI, "", $g_hSQLRichEdit, $g_xCtrl, $g_yCtrl, $g_wCtrl, 150)
            
            _WinAPI_UpdateWindow($g_hSQLRichEdit)
            _WinAPI_RedrawWindow($g_hSQLRichEdit, 0, 0, BitOR($RDW_INVALIDATE, $RDW_UPDATENOW, $RDW_FRAME))
            _LogInfo("RichEdit-Control wurde angezeigt mit Größe: " & $g_wCtrl & "x150")
        Else
            _LogError("Fehler: RichEdit-Control ist ungültig!")
        EndIf
        
        ; SQL für aktuelle Tabelle vorbereiten
        _PrepareCurrentSQL()
        
        ; Nochmals versuchen, das SQL-Statement explizit zu setzen
        If $g_sCurrentTable <> "" Then
            _SQL_SetInitialValue()
        EndIf
...
Func _PrepareCurrentSQL()
    ...
    If $sCurrentTable <> "" Then
        ...
        ; Statement in Editor setzen (mit zusätzlichen Prüfungen)
        If IsHWnd($g_hSQLRichEdit) Then 
            _LogInfo("Verwende _ForceSetRichEditText in _PrepareCurrentSQL")
            _ForceSetRichEditText($g_hSQLRichEdit, $sSQL)
        Else
    ...
    Switch $iMsg
        Case $g_idTableCombo, $idTableCombo
            ...
            If $sTable <> "" Then
                ...
                ; Spezielle Funktion zum zuverlässigen Setzen des SQL-Texts
                If Not _ForceSetSQLText($sSQL) Then
                    _LogError("Fehler beim Setzen des SQL-Texts im Editor")
                EndIf
                
                ; Spalten für Autovervollständigung laden
                _LogInfo("Lade Spalten für Tabelle '" & $sTable & "'")
                ; Verwende den SQL-Autocomplete-Connector
                _SQL_UpdateAutoCompleteMetadata($g_sCurrentDB, $sTable)
                _LogInfo("Metadaten für Autovervollständigung aktualisiert")
                
                ; Syntax aktualisieren und Benutzerhinweis
                _SQL_UpdateSyntaxHighlighting()
                _SetStatus("SQL-Statement für Tabelle '" & $sTable & "' wurde erstellt. Klicken Sie auf 'Ausführen' um die Daten zu laden.")
                
                ; Sicherstellen, dass das RichEdit sichtbar ist
                If IsHWnd($g_hSQLRichEdit) Then
                    _WinAPI_ShowWindow($g_hSQLRichEdit, @SW_SHOW)
                    _WinAPI_UpdateWindow($g_hSQLRichEdit)
                    _WinAPI_RedrawWindow($g_hSQLRichEdit, 0, 0, BitOR($RDW_INVALIDATE, $RDW_UPDATENOW))
                    
                    ; Fokus auf SQL-Editor setzen und Cursor ans Ende
                    _WinAPI_SetFocus($g_hSQLRichEdit)
                    _GUICtrlRichEdit_SetSel($g_hSQLRichEdit, StringLen($sSQL), StringLen($sSQL))
                EndIf
            EndIf
            Return True
